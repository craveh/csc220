...
public class IntTreeTester {
	public static void main(String[] args){
		...
		//tree2.printSideways();
		//tree_ref2.printSideways();
		System.out.println("tree_ref3");
		tree_ref3.printSideways();
		//tree_ref4.printSideways();

		// *** perfectify() tests
		empty_tree.perfectify();
		if (!empty_tree.getInorder().equals(""))
			System.err.println("TEST 0 FAILED: perfectify() with empty tree");

		tree1.perfectify();
		if (!tree1.getInorder().equals(" 1 "))
			System.err.println("TEST 1 FAILED: perfectify() with tree1");

		tree2.perfectify();
		if (!tree2.getInorder().equals(" 2  1  -1 "))
			System.err.println("TEST 2 FAILED: perfectify() with tree2");

		tree3.perfectify();
		if (!tree3.getInorder().equals(" 2  1  3 "))
			System.err.println("TEST 3 FAILED: perfectify() with tree3");

		tree4.perfectify();
		System.out.println(tree4.getInorder());
		if (!tree4.getInorder().equals(" 4  2  -1  1  -1  3  -1 "))
			System.err.println("TEST 4 FAILED: perfectify() with tree4");

		tree6.perfectify();
		if (!tree6.getInorder().equals(" 4  2  5  1  6  3  -1 "))
			System.err.println("TEST 5 FAILED: perfectify() with tree6");

		tree_ref1.perfectify();
		if (!tree_ref1.getInorder().equals(" 1  5  -1  3  4  2  6 "))
			System.err.println("TEST 6 FAILED: perfectify() with tree_ref1");

		tree_ref2.perfectify();
		if (!tree_ref2.getInorder().equals(" -1  0  -1  8  -1  -1  -1  2  4  7  -1  1  -1  6  9 "))
			System.err.println("TEST 7 FAILED: perfectify() with tree_ref2");
