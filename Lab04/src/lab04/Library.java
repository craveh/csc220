...
public class Library<Type> {
  ...
  protected class IsbnComparator implements Comparator<LibraryBook<Type>> {
	// FILL IN - write the compare method
	  public int compare(LibraryBook<Type> bookL, LibraryBook<Type> bookR) {
		  if(bookL.getIsbn()-bookR.getIsbn() > 0) {
			  return 1;
		  }else{
			  return -1;
		  }
  ...
  public ArrayList<LibraryBook<Type>> getOverdueList(int month, int day,
    ...
    // FILL IN -- do not return null
	  ...
	  for(int index = 0; index<library.size(); index++) {
		  if(library.get(index).getDueDate() != null) {
			  if(date.compareTo(library.get(index).getDueDate()) >= 0){
				  overdueList.add(library.get(index));
			  }
		  }
	  }
	  if(overdueList.size() != 0) {
		  DueDateComparator dueDateComp = new DueDateComparator();
		  sort(overdueList, dueDateComp);
		  return overdueList;
...
Comparator<LibraryBook<Type>> {
	  ...
	  public int compare(LibraryBook<Type> bookL, LibraryBook<Type> bookR) {
		  ...
		  if(bookL.getAuthor().compareTo(bookR.getAuthor())  > 0 ){
			  return 1;
		  }else if(bookL.getAuthor().compareTo(bookR.getAuthor()) < 0) {
			  return -1;
		  }else {
			  if(bookL.getTitle().compareTo(bookR.getTitle()) > 0) {
				  return 1;
			  }else {
				  return -1;
			  }
  ...
  protected class DueDateComparator implements Comparator<LibraryBook<Type>> {
	// FILL IN - write the compare method
	  public int compare(LibraryBook<Type> bookL, LibraryBook<Type> bookR) {
		  if(bookL.getDueDate().compareTo(bookR.getDueDate()) > 0) {
			  return 1;
		  }else {
			return -1;  
		  }
