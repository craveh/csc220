...
public class Library {
  ...
  public String lookup(long isbn) {
    // *FILL IN -- do not return null unless appropriate
	  int index;
	  //while(library.indexOf(index).getIsbn)
	  //long numbers = library.get(0).getIsbn();
  ...
  public boolean checkout(long isbn, String holder, int month, int day, int year) {
    // *FILL IN -- do not return false unless appropriate
	  ...
	  for(index = 0; index<library.size(); index++) {
		  if (library.get(index).getIsbn() == isbn){
			  if(library.get(index).holder == null) {
				  GregorianCalendar dueDate = new GregorianCalendar(year, month, day);
				  library.get(index).checkout(holder, dueDate);
				  return(true); 
			  }
		  }
	  }
  ...
  public ArrayList<LibraryBook> lookup(String holder) {
    // FILL IN -- do not return null
	  ...
	  for(index = 0; index<library.size(); index++) {
		  if(library.get(index).holder == holder) {
			  holdersBooks.add(library.get(index));
			  }
